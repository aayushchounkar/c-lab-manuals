...
public:
    ...
    void cart(int choice) {
        if (choice == 1) {
            ...
            price = 230;
            description = "A Full meal Thali Served with 4 Rotis and Jeera Rice alongside With salad, paneer, Dal, papad";
        } else if (choice == 2) {
...
void foodMenu() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            l.push_back(des.name + "  for INR " + to_string(des.price) + "          " + des.description);
            price += des.price;
        } else if (choice == 2) {
            cout << "Your Item : ";
            cout << "\n           " << app.name << "  for INR " << app.price << "\n           " << app.description << endl;
            l.push_back(app.name + "  for INR " + to_string(app.price) + "           " + app.description);
            price += app.price;
        } else if (choice == 3) {
            cout << "Your Item : \n\n           " << mncr.name << "  for INR " << mncr.price << "\n           " << mncr.description << endl;
            l.push_back(mncr.name + "  for INR " + to_string(mncr.price) + "           " + mncr.description);
            price += mncr.price;
        }
        ...
        if (y[0] != 'Y' && y[0] != 'y') {
            ...
            cout << "\nTotal Bill = INR " << price + (price * 0.18) << " + (TAX) " << price * 0.18 << endl;
            cout << "\nGRAND TOTAL  = INR " << price + (price * 0.18) << endl;
            ...
            if (pay == "ONLINE" || pay == "CASH") {
                if (pay == "ONLINE") {
                    cout << "\nEnter Upi id : ";
                    cin >> id;
                    cout << "\nSent the request to pay " << price + (price * 0.18) << " to your upi id " << id << endl;
                } else {
                    cout << "\nPlease pay " << price + (price * 0.18) << " with the cash With Change......Thank you :)" << endl;
                }
            }
